...
// Primary config, or defaults.app_httpd.cpp also uses ssid,so __has_include is necessary
...
void WifiSetup() {
    ...
    if (stationCount > firstStation) {
        ...
        //int stationsFound = WiFi.scanNetworks(false,true);//scan hidden ssid
        int stationsFound = WiFi.scanNetworks(false,true,false,300,0,"OrayBox-81B4");//scan hidden ssid
...
void setup() {
    Serial.begin(115200);
    bat_init();
    bat_hold_output();//enable battery power when the usb supplied power is unplugged.
    vol=bat_get_voltage();//display voltage via serial monitor
...
void loop() {
	Serial.printf(vol);//display voltage via serial monitor
}
